...
class clsIndexBase extends clsBase
{
  function Formular()
  {
    $this->SetTitulo($this->_instituicao . ' Trilha Jovem - Funcionário');
    $this->processoAp = 635;
  }
}
...
class indice extends clsCadastro
{
  ...
  function Gerar()
  {
    ...
    if ($this->cod_servidor) {
      ...
      $this->campoRotulo('nm_servidor', 'Funcionário', $detalhe['nome']);
      ...
      $this->campoListaPesq('cod_servidor', 'Funcionário', $opcoes,
    ...
    $this->campoTabelaInicio('funcao', 'Funções Funcionário',
    ...
    if ($this->docente) {
      ...
      if (isset($docenteInep)) {
        $this->campoRotulo('_inep_cod_docente', 'Código do educador no Educacenso/Inep',
          $docenteInep->docenteInep);

        if (isset($docenteInep->nomeInep)) {
          $this->campoRotulo('_inep_nome_docente', 'Nome do educador no Educacenso/Inep',
  ...
  function Excluir()
  {
    ...
    if ($obj_quadro_horario->detalhe()) {
      $this->mensagem = "Exclusão não realizada. O funcionário está vinculado a um quadro de horários.<br>";
      return FALSE;
    }
    else {
      $obj_quadro_horario = new clsPmieducarQuadroHorarioHorarios(NULL, NULL,
...
function atualizaLstFuncao(xml)
{
  ...
  if (funcoes.length) {
    ...
    campoFuncao.options[0].text = 'A instituição não possui funções de funcionários';
  }
}
