...
class indice extends clsDetalhe
{
	...
	function Gerar()
	{
		...
		{
			$this->addDetalhe( array( "Número de Ocorrências", "{$registro["sequencial"]}") );
		}
		if( $registro["data_cadastro"] )
		{
			if($hora = dataFromPgToBr("{$registro["data_cadastro"]}",'H:i') )
				$this->addDetalhe( array( "Hora da Ocorrência", $hora ));
			$this->addDetalhe( array( "Data Ocorrência", dataFromPgToBr("{$registro["data_cadastro"]}",'d/m/Y') ));
		}
		if( $registro["ref_cod_tipo_ocorrencia_disciplinar"] )
		{
			$this->addDetalhe( array( "Tipo Ocorrência", "{$registro["nm_tipo"]}") );
		}
		if( $registro["observacao"] )
		{
			$this->addDetalhe( array( "Observações", nl2br("{$registro["observacao"]}")) );
		}
