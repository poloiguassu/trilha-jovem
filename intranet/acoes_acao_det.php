...
class clsIndex extends clsBase
{
	...
	function Formular()
	{
		$this->SetTitulo( "Trilha Jovem Iguassu - Sistema de Cadastro de Ações - Detalhe de ações" );
		$this->processoAp = "551";
	}
}
...
class indice extends clsDetalhe
{
	...
	function Gerar()
	{
		...
		if($det_acao_governo['numero_acao'])
			$this->addDetalhe( array("Número da ação", "{$det_acao_governo['numero_acao']}") );
		$this->addDetalhe( array("Nome da ação", "{$det_acao_governo['nm_acao']}") );
		$this->addDetalhe( array("Descrição da ação", "{$det_acao_governo['descricao']}") );
		...
		$this->addDetalhe( array("Destaque",$det_acao_governo['destaque'] == 0 ? "Não" : "Sim"));
		...
		if($det_acoes){
			...
			$this->addDetalhe(array("Detalhes da Ação", "<a href='$func' >Mostrar detalhe</a><div id='det_pree' name='det_pree' style='display:{$display};'>".$det_acoes."</div>"));
		}
		...
		if($lista_fotos)
		{
			...
			if($lista_fotos)
			{
				...
				while ($db->ProximoRegistro())
				{
					...
					if($descricao){
						$rowspan = "rowspan='2'";
						$descricao = "<tr bgcolor=$cor><td colspan='2'><div><b>Descrição:</b> {$descricao}</div></td></tr>";
					}
