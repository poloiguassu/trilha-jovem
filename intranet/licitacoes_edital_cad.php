...
class indice extends clsCadastro
{
	...
	function Gerar()
	{
		...
		if( isset( $this->ref_pessoa ) )
		{
			...
			$this->campoRotulo( "versao", "Versão",  ++$versao  );
			...
			{
				$this->campoRotulo( "visivel", "Não Ocultar", "<b>Edital oculto</b> - <a href=\"licitacoes_edital_cad.php?cod_edital={$_GET["cod_edital"]}&ocultar=0\">Clique aqui para exibir este Edital na lista de downloads.</a>" );
			}

			$this->campoMemo( "motivo", "Motivo da alteração", $this->motivo, 50, 8, true );

			$this->campoOculto( "ref_licitacao", $this->ref_licitacao );
		}
		else
		{
			$this->campoRotulo( "versao", "Versão", 1 );
			$lista = array();
			$db = new clsBanco();
