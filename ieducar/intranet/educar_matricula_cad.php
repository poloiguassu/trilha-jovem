...
class clsIndexBase extends clsBase
{
  function Formular()
  {
    $this->SetTitulo($this->_instituicao . ' Trilha Jovem - Matrícula');
    $this->processoAp = 578;
  }
}
    ...
    if(is_array($anoLetivoEmAndamentoEscola)) {
      ...
      if (is_array($m) && count($m)) {
        ...
        if ($m['ref_ref_cod_serie'] == $this->ref_cod_serie) {
          $this->mensagem .= "Este aluno já está matriculado neste eixo e projeto, não é possivel matricular um aluno mais de uma vez no mesmo eixo.<br />";
        ...
        elseif ($curso['multi_seriado'] != 1) {
          ...
          $this->mensagem .= "Este aluno já está matriculado no(a) '$nomeSerie' deste projeto e instituição. Como este projeto não é multi seriado, não é possivel manter mais de uma matricula em andamento para o mesmo projeto.<br />";
      ...
      else
      {
        ...
        if (is_array($m) && count($m)){
          ...
          $this->mensagem .= "Este aluno já está matriculado no(a) '$serie' do projeto '$curso' na instituição '$escola', para matricular este instituição na sua escola solicite transferência a coordenação(a) da instituição citada.<br />";
      ...
      if (! $this->ref_cod_reserva_vaga) {
        ...
        if (is_array($lst_turmas)) {
          ...
          $this->mensagem = 'O eixo selecionada não possui turmas cadastradas.<br />';
          return FALSE;
        }
      ...
      $this->mensagem = 'O ano selecionado não esta em andamento na instituição selecionada.<br />';
      return FALSE;
    }
  }
