...
class indice extends clsListagem
{
	...
	function Gerar()
	{
		...
		$cabecalhos = array("Instituição");
		$nivel = $obj_permissoes->nivel_acesso($this->pessoa_logada);
		if( $nivel == 1 )
		{
			$cabecalhos[] = "Instituição Executora";
			...
			$this->campoLista( "ref_cod_instituicao", "Instituição Executora", $opcoes, $this->ref_cod_instituicao, false, false, false, false, false, false );
		}
		else
		{
			...
			{
				die( "Erro: Usuário não é do nivel poli-institucional ou não possui uma instituição" );
			}
		}
		$this->addCabecalhos( $cabecalhos );

		$this->campoTexto( "nm_escola", "Instituição", $this->nm_escola, 30, 255, false );
