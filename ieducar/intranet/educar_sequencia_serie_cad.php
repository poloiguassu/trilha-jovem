...
class clsIndexBase extends clsBase
{
	function Formular()
	{
		$this->SetTitulo( "{$this->_instituicao} Trilha Jovem - Enturmação" );
		$this->processoAp = "587";
	}
}
...
//		echo "<pre>"; print_r($GLOBALS); die();
			...
			$this->campoLista( "ref_cod_instituicao", "Instituição Executora", $opcoes, $this->ref_cod_instituicao );
		}
		else
		{
...
function getCurso(xml_curso)
{
	...
	*/
	...
	else
	{
		...
		campoCurso_.options[0].text = 'A instituição não possui nenhum projeto';
	}
}
...
function getSerie(xml_serie)
{
	...
	if(DOM_array.length)
	{
		...
		campoSerie.options[0].text = 'O projeto origem não possui nenhum eixo';
}
...
function getSerie_(xml_serie_)
{
	...
	if(DOM_array.length)
	{
		...
		campoSerie_.options[0].text = 'O projeto origem não possui nenhum eixo';
}
/*
