		...
		if ( $nivel_usuario == 1 )
		{
			...
			if ( $get_escola && $get_curso )
			{
				$retorno .= '<tr id="tr_status">
							 ...
							 <span class="form">Instituição Executora</span>
...
//							}
			...
			else
			{
				...
				$opcoes_curso = array( "" => "Erro na geração" );
			}
...
*/
			...
			else
			{
				...
				$todas_series = array( "" => "Erro na geração" );
			}
			$retorno .= '<tr id="tr_curso">
						 <td valign="top" class="formmdtd">
		...
		if ( $get_turma )
		{
			...
			else
			{
				...
				$todas_turmas = array( "" => "Erro na geração" );
			}
		...
		if ( $nivel_usuario == 1 )
		{
			$validacao = 'if ( !document.getElementById( "ref_cod_instituicao" ).value ) {
					alert( "Por favor, selecione uma instituição executora" );
					return false;
					}
...
?>
	function getEscola( xml_escola )
	{
		...
		if(DOM_array.length)
		{
			...
			campoEscola.options[0].text = 'A instituição executora não possui nenhuma instuituição';
	}
<?
}
?>
...
function getSerie( xml_serie )
{
	...
	if(DOM_array.length)
	{
		...
		campoSerie.options[0].text = 'A projeto não possui nenhum eixo';
}
