...
class clsIndexBase extends clsBase
{
	function Formular()
	{
		$this->SetTitulo( "{$this->_instituicao} Trilha Jovem - Funções de Funcionários" );
		$this->processoAp = "634";
	}
}
...
class indice extends clsDetalhe
{
	...
	function Gerar()
	{
		...
		{
			$this->addDetalhe( array( "Função", "{$registro["cod_funcao"]}") );
		}
		if( $registro["nm_funcao"] )
		{
			$this->addDetalhe( array( "Nome da Função", "{$registro["nm_funcao"]}") );
		}
		...
		{
			$this->addDetalhe( array( "Educador", "{$opcoes[$registro["professor"]]}") );
		}
