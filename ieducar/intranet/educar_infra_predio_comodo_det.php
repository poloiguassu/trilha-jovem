...
class clsIndexBase extends clsBase
{
	function Formular()
	{
		$this->SetTitulo( "{$this->_instituicao} Trilha Jovem - Comôdos" );
		$this->processoAp = "574";
	}
}
...
class indice extends clsDetalhe
{
	...
	function Gerar()
	{
		...
		{
			$this->addDetalhe( array( "Prédio", "{$registro["ref_cod_infra_predio"]}") );
		}
		if( $registro["nm_comodo"] )
		{
			$this->addDetalhe( array( "Cômodo", "{$registro["nm_comodo"]}") );
		}
		if( $registro["ref_cod_infra_comodo_funcao"] )
		{
			$this->addDetalhe( array( "Função Cômodo", "{$registro["ref_cod_infra_comodo_funcao"]}") );
		}
		if( $registro["area"] )
		{
			$this->addDetalhe( array( "Área", "{$registro["area"]}") );
		}
		if( $registro["desc_comodo"] )
		{
			$this->addDetalhe( array( "Descrição do Cômodo", "{$registro["desc_comodo"]}") );
		}
