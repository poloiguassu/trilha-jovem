...
class clsIndexBase extends clsBase
{
	function Formular()
	{
		$this->SetTitulo( "{$this->_instituicao} Trilha Jovem - Cômodos" );
		$this->processoAp = "574";
	}
}
...
class indice extends clsCadastro
{
	...
	function Gerar()
	{
		...
		$this->campoLista( "ref_cod_infra_comodo_funcao", "Funcão Cômodo", $opcoes_funcao, $this->ref_cod_infra_comodo_funcao,"", false, "", $script );



		// text
		$this->campoTexto( "nm_comodo", "Cômodo", $this->nm_comodo, 43, 255, true );
		$this->campoMonetario("area", "Área", $this->area, 10, 255, true );
		$this->campoMemo( "desc_comodo", "Descrição Comodo", $this->desc_comodo, 60, 5, false );
	}
	...
	function Editar()
	{
		...
		$this->mensagem = "Edição realizada.<br>";
...
function getInfraPredio(xml_infra_predio)
{
	...
	*/
	...
	if(DOM_array.length)
	{
		...
		campoPredio.options[0].text = 'A instituição não possui prédio cadastrado';
...
function getInfraPredioFuncao(xml_infra_comodo_funcao)
{
	...
	*/
	...
	{
		campoFuncao.length = 1;
		campoFuncao.options[0].text = 'Selecione uma função ao cômodo';
		...
		campoFuncao.options[0].text = 'A instituição não possui função ao cômodo';
}
