...
class indice extends clsDetalhe
{
	...
	function Gerar()
	{
		...
		{
			$this->addDetalhe( array( "Localização", "{$registro["ref_cod_escola_localizacao"]}") );
		}
		...
		if( $registro["ref_idpes"] )
		{
			$this->addDetalhe( array( "Razão Social", "{$registro["ref_idpes"]}") );
		}
		if ($tipo == 1)
		{
			if( $cep )
			...
			if( $numero )
			{
				$this->addDetalhe( array( "Número", "{$numero}") );
			}
			if( $andar )
			{
	...
	function listaAnos()
	{
		...
		if($lista_ano_letivo)
		{
			...
			$existe  = true;
			$tabela .= "<tr bgcolor=$cor><td colspan='2'><b>Etapas em Execução</b></td></tr><tr><td>";
			...
			$tabela .= "<tr>
							<td>
								<span class='formlttd'><b>*Somente é possível finalizar uma etapa em execução após não existir mais nenhuma matrícula em andamento.</b></span>
							</td>
						</tr>";
			$tabela .= "<tr>
							<td>
