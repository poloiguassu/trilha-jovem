...
class indice extends clsListagem
{
	...
	function Gerar()
	{
		...
		$this->titulo = "Enturmação - Listagem";
		...
		if( $nivel_usuario == 1 )
		{
			...
			$this->campoLista( "ref_cod_instituicao", "Instituição executora", $opcoes, $this->ref_cod_instituicao, "",null,null,null,null,false );
		}
		else
		{
		...
		if( is_array( $lista ) && count( $lista ) )
		{
			foreach ( $lista AS $registro )
			{
				...
				if( class_exists( "clsPmieducarSerie" ) )
				{
					...
					{
						$registro["ref_cod_origem"] = "Erro na geração";
						echo "<!--\nErro\nClasse não existente: clsPmieducarCurso\n-->";
					}
				}
				else
				{
					$registro["ref_serie_origem"] = "Erro na geração";
				...
				if( class_exists( "clsPmieducarSerie" ) )
				{
					...
					{
						$registro["ref_cod_destino"] = "Erro na geração";
						echo "<!--\nErro\nClasse não existente: clsPmieducarCurso\n-->";
					}
				}
				else
				{
					$registro["ref_serie_destino"] = "Erro na geração";
					echo "<!--\nErro\nClasse não existente: clsPmieducarSerie\n-->";
				}
