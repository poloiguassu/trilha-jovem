...
class BoletimController extends Core_Controller_Page_ViewController
{
  ...
  public function Gerar()
  {
    ...
    $this->addDetalhe(array('Instituição', $escola));
    $this->addDetalhe(array('Projeto', $curso));
    $this->addDetalhe(array('Eixo/Turma', $serie . ' / ' . $turma));
    ...
    if ($nenhumaNota) {
      ...
      $linkText = ($porComponente ? 'avaliação/falta' : 'avaliação');
      $linkPath = 'nota';
    }
    ...
    foreach ($componentes as $id => $componente) {
      ...
      if ($porComponente == TRUE) {
        $new = $url->l('Lançar Avaliação', 'nota',
          array('query' =>
