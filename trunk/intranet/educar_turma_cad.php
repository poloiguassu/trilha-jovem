...
class indice extends clsCadastro
{
  ...
  function Gerar()
  {
    ...
    $this->campoListaPesq('ref_cod_regente', 'Educador Coordenador', $array_servidor,
    ...
    $label = 'Componentes curriculares:<br />'
           ...
           . 'da eixo/projeto da instituição:'
  ...
  protected function getCountMatriculas($escolaId, $turmaId) {
    ...
    if (! is_numeric($ano)) {
      $this->mensagem = "Não foi possivel obter um ano em andamento, por favor, inicie um ano para a instituição ou desative a configuração (para eixo e instituição) 'Bloquear cadastro de novas turmas antes de atingir limite de vagas (no mesmo turno)'.";
...
function atualizaTurmaCad_TipoComodo(xml)
{
  ...
  if (tipo_comodo.length) {
    ...
    campoComodo.length = 1;
    campoComodo.options[0] = new Option('A instituição não possui nenhuma Sala', '', false, false);
  }
}
...
function atualizaMultiSerie(xml)
{
  ...
  campoSerieMult.options[0] = new Option('Selecione um eixo', '', false, false);
  ...
  if (campoSerieMult.length == 1 && campoCurso != '') {
    campoSerieMult.options[0] = new Option('O projeto não possui nenhum eixo', '', false, false);
  }
...
function valida_xml(xml)
{
  ...
  if (document.getElementById('ref_cod_escola').value) {
    if (!document.getElementById('ref_ref_cod_serie').value) {
      alert("Preencha o campo 'Eixo' corretamente!");
  ...
  if (document.getElementById('multiseriada').checked) {
    if (!document.getElementById('ref_ref_cod_serie_mult')){
      alert("Preencha o campo 'Eixo Multi-seriado' corretamente!");
...
function atualizaLstEscolaCursoSerie(xml)
{
  var campoSerie             = document.getElementById('ref_ref_cod_serie');
  campoSerie.length          = 1;
  campoSerie.options[0].text = 'Selecione uma eixo';
