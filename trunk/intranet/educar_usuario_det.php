...
class clsIndexBase extends clsBase
{
	function Formular()
	{
		$this->SetTitulo( "{$this->_instituicao} i-Educar - Usuário" );
		$this->processoAp = "555";
	}
}
...
class indice extends clsDetalhe
{
	...
	function Gerar()
	{
		...
		{
			$registro["cod_usuario"] = "Erro na geração";
			echo "<!--\nErro\nClasse não existente: clsFuncionario\n-->";
		}
		...
		{
			$registro["ref_cod_tipo_usuario"] = "Erro na geração";
		...
		if( $nm_usuario )
		{
			$this->addDetalhe( array( "Usuário", "{$nm_usuario}") );
		}
		if( $registro["ref_cod_tipo_usuario"] )
		{
			$this->addDetalhe( array( "Tipo Usuário", "{$registro["ref_cod_tipo_usuario"]}") );
		}
