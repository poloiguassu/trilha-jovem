...
class clsIndexBase extends clsBase
{
	function Formular()
	{
		$this->SetTitulo( "{$this->_instituicao} Cadastro de Funcionários" );
		$this->processoAp = "36";
	}
}
...
class indice extends clsCadastro
{
	...
	function Gerar()
	{
		...
		$opcoes = array("" => "Selecione", 5 => "Comissionado", 4 => "Contratado", 3 => "Efetivo", 6 => "Estagiário");
		...
		$this->campoCheck("super", "Super usuário", $this->super);
		$this->campoCheck("proibido", "Banido", $this->proibido);
		$this->campoCheck("matricula_permanente", "Matrícula permanente", $this->matricula_permanente);

		//-----------------------------------------------------------------------------------------------

		$this->campoRotulo("rotulo_permissoes", "<b><i>Permissões</i></b>", "");
				...
				if(is_array($_POST[str_replace(" ", "_",limpa_acentos(strtolower($menu["nm_menu"]))."_")]) && count($_POST[str_replace(" ", "_",limpa_acentos(strtolower($menu["nm_menu"]))."_")]))
				{
					...
					foreach ($_POST[str_replace(" ", "_",limpa_acentos(strtolower($menu["nm_menu"])."_"))] as $ind => $id_submenu)
					{
						if($id_submenu)
						{
							...
							{
								$this->mensagem = "Cadastro de menu não realizado.<br>";
    ...
    if (! $this->validatesPassword($this->matricula, $this->_senha))
      return false;
		...
		if( $obj_funcionario->cadastra() )
		{
			...
			$this->mensagem = "Cadastro de menus não realizado.<br>";
			echo "<!--\nErro ao cadastrar clsPortalMenuFuncionario-->";
			return false;
		}
