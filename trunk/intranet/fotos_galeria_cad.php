...
class indice extends clsCadastro
{
	...
	function Inicializar()
	{
		...
		if (@$_GET['id_foto'])
		{
			...
			$db = new clsBanco();
			$db->Consulta( "SELECT f.ref_ref_cod_pessoa_fj, f.cod_foto_portal, f.ref_cod_foto_secao, f.titulo, f.descricao, f.data_foto, f.caminho, f.nm_credito, f.altura, f.largura, f.ref_cod_foto_secao FROM foto_portal f WHERE cod_foto_portal=$this->id_foto" );
			if ($db->ProximoRegistro())
			{
				list($this->cod_pessoa, $this->id_foto, $this->ref_cod_foto_secao, $this->titulo, $this->descricao, $this->data_foto, $this->foto, $this->nm_credito, $this->altura, $this->largura, $this->secao ) = $db->Tupla();
		...
		$this->url_cancelar = ($retorno == "Editar") ? "fotos_galeria_det.php?id_foto=$this->id_foto" : "fotos_galeria_lst.php";
	...
	function Gerar()
	{
		...
		$this->campoRotulo( "pessoa", "Responsável", $this->nome_);
		$this->campoTexto( "titulo", "Título", $this->titulo, "50", "100", true );
	...
	function Novo() 
	{
		...
		if ( !empty($HTTP_POST_FILES['foto']['name']) )
		{
			...
			$nome_do_arquivo = date('Y-m-d')."-".substr(md5($fotoOriginal), 0, 10).$extensao;
			$caminhoDaBig = "fotos/big/{$nome_do_arquivo}";
			$caminhoDaSBig = "fotos/sbig/{$nome_do_arquivo}";
			
			if ($imagewidth > 400 && $imageheight<$imagewidth)
			{
				$new_w = 400;
			...
			elseif ($imagewidth > 250 && $imageheight>$imagewidth)
			{
				$new_w = 250;
				$ratio = ($imagewidth / $new_w);
				$new_h = ceil($imageheight / $ratio);
		...
		$db = new clsBanco();
		$db->Consulta( "INSERT INTO foto_portal ( nm_credito, ref_cod_foto_secao, ref_ref_cod_pessoa_fj, data_foto, titulo, descricao, caminho, altura, largura ) VALUES ( '{$this->nm_credito}', '$this->secao',  {$this->id_pessoa}, now(), '{$this->titulo}', '{$this->descricao}', '{$nome_do_arquivo}', $fim_largura, $fim_altura )" );

		echo "<script>document.location='fotos_galeria_lst.php';</script>";
