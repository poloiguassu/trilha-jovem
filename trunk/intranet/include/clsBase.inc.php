...
{
  var $titulo     = 'Trilha Jovem Iguassu';
  var $clsForm    = array();
  var $bodyscript = NULL;
  var $processoAp;
  var $refresh    = FALSE;
  ...
  function MakeHeadHtml()
  {
    ...
    if ($this->refresh) {
      $saida = str_replace("<!-- #&REFRESH&# -->", "<meta http-equiv='refresh' content='60; charset=utf-8'>", $saida);
    }
  ...
  function VerificaPermicao()
  {
    if(is_array($this->processoAp))
    {
      ...
      if (!$permite) {
        ...
        die("Acesso negado para este usuário");
      }
    }
    else {
    ...
    if ($menu_tutor) {
      ...
      if ($lista_menu_suspenso) {
        ...
        foreach ($lista_menu as $menu_suspenso) {
		  ...
		  $nome_menu = $menu_suspenso['tt_menu'];
		  
          $saida .= "array_menu[array_menu.length] = new Array(\"{$nome_menu}\",{$menu_suspenso['cod_menu']},'{$menu_suspenso['ref_cod_menu_pai']}','', '$ico_menu', '{$menu_suspenso['caminho']}', '{$alvo}');";
