...
class indice extends clsListagem
{
	...
	function Gerar()
	{
		...
		{
			echo "<!--\nErro\nClasse clsMunicipio não encontrada\n-->";
			$opcoes = array( "" => "Erro na geração" );
		}
		...
		{
			echo "<!--\nErro\nClasse clsUrbanoTipoLogradouro não encontrada\n-->";
			$opcoes = array( "" => "Erro na geração" );
		}
		...
		if( is_array( $lista ) && count( $lista ) )
		{
			foreach ( $lista AS $registro )
			{
				...
				{
					$registro["idtlog"] = "Erro na geração";
					echo "<!--\nErro\nClasse não existente: clsUrbanoTipoLogradouro\n-->";
				}
...
function getUf( xml_uf )
{
	...
	{
		campoUf.options[0].text = 'Este País não possui nenhum estado';
	}
}
