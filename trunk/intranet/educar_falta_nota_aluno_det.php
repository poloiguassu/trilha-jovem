...
//		echo $this->ref_cod_matricula." ".$this->ref_cod_turma;
		...
		if( $qtd_modulos )
		{
			$this->addDetalhe( array( "Quantidade de Módulos", "{$qtd_modulos}") );
		}
			...
			if (is_array($lst_disciplinas))
			{
				...
				for ( $i = 1; $i <= $max_qtd_nota; $i++ )
				{
					if ($qtd_modulos < $i)
						...
						$tabela .= "<td colspan='2' bgcolor='#a1b3bd'><b>Módulo {$i}</b></td>";
				}
				$tabela .= "</tr>";
				...
				{
					if ($qtd_modulos < $i)
						$tabela .= "<td colspan='2' bgcolor='#a1b3bd'><b>Avaliação</b></td>";
					else
...
//					echo "<pre>"; print_r($lst_disciplinas); die();
	...
	//************************************* DETALHE - MATRICULADO NUM CURSO *************************************//
		...
		if( $aprovado )
		{
			...
			$this->addDetalhe( array( "Situação", "{$aprovado_}") );
		}
